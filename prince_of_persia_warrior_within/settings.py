...
class GUI:
    ...
    def config_load(self):
        ...
        config_file = current_dir + '/game/pop2.ini'
        ...
        if not config_parser.has_option('HUD', 'hud_glitch_fix'):
            self.hud_glitch_fix = '1'
            config_parser.set('HUD', 'hud_glitch_fix', self.hud_glitch_fix)
        else:
            self.hud_glitch_fix = config_parser.get('HUD', 'hud_glitch_fix').split(' // ')[0]
        ...
        if not config_parser.has_option('MISC', 'Xbox_fov'):
            self.xbox_fov = '1'
            config_parser.set('MISC', 'Xbox_fov', self.xbox_fov)
        else:
    ...
    def create_main_window(self):
        ...
        self.main_window = Gtk.Window(
            title = _("Prince of Persia: Warrior Within"),
        ...
        label_hud_glitch_fix = Gtk.Label(
            label = _("HUD glitch fix:"),
            halign = Gtk.Align.START
            )

        switch_hud_glitch_fix = Gtk.Switch(
            halign = Gtk.Align.END
            )
        if self.hud_glitch_fix == '1':
            switch_hud_glitch_fix.set_active(True)
        else:
            switch_hud_glitch_fix.set_active(False)
        switch_hud_glitch_fix.connect('state-set', self.cb_switch_hud_glitch_fix)
        ...
        grid.attach(label_hud_glitch_fix, 0, 4, 1, 1)
        grid.attach(switch_hud_glitch_fix, 1, 4, 2, 1)
    ...
    def cb_switch_hud_glitch_fix(self, switch, event):
        if switch.get_active():
            self.hud_glitch_fix = '1'
        else:
            self.hud_glitch_fix = '0'
